				...
				<table class="table table-condensed">
					...
					<tbody>
						<c:forEach items="${allDept }" var="dept">
							<tr>
								<td class="text-center"><input type="text" id="dname-${dept.did }" class="form-control" value="${dept.dname }" style="width:400px;"></td>
								<td class="text-center"><input type="text" id="maxnum-${dept.did }" class="form-control" value="${dept.maxnum }" style="width:100px;"></td>
								<td class="text-center"><span id="eid-${dept.eid }" style="cursor:pointer;">${allEmpByDeptEid[dept.eid] } </span></td> 
								<td class="text-center"><span id="currnum-${dept.did }" class="text-danger">${dept.currnum }</span></td>
								<td class="text-center">
									<button id="edit-${dept.did }" class="btn btn-warning">
											<span class="glyphicon glyphicon-edit"></span>&nbsp;编辑${dname }</button>
								</td>
								<td>${emp.ename }</td>
							</tr>
						</c:forEach>
