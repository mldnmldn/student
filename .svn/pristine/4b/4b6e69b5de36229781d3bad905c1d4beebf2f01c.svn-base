...
<%!
	public static final String SCHEDULE_ADD_URL = "pages/back/admin/schedule/ScheduleAddAction!AddSchedule.action" ;
	...
	<div class="wrapper">
		...
		<div class="content-wrapper text-left">
				...
				<div class="panel-body">
					<form class="form-horizontal" action="<%=SCHEDULE_ADD_URL%>" id="myform" method="post">
						<fieldset>
								...
								<div class="col-md-5">
									<select id="iid" name="iid" class="form-control">
										...
										<c:forEach items="${allItems}" var="item">
											<c:if test="${item.iid==vo.iid}">
												<option value="${item.iid}" id="${item.iid}" cheched>${item.title}</option>
											</c:if>
											<c:if test="${item.iid!=vo.iid}">
							...
							<div class="form-group">
								<div class="col-md-5 col-md-offset-3">
									<button id="addButton" class="btn btn-primary">增加</button>
