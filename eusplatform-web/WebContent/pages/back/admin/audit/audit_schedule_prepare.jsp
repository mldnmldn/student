...
<%!
	...
	public static final String SCHEDULE_SHOW_URL = "pages/back/admin/schedule/ScheduleActionFront!show.action" ;//修改后跳转的页面pages/back/admin/schedule/ScheduleActionFront!show.action
%>
						...
						<tbody>
							<c:forEach items="${allSchedule}" var="schedule">
									...
									<td class="text-center">
										<a id="showBtn-2" onmouseover="this.style.cursor='hand'" href="<%=SCHEDULE_SHOW_URL%>?sid=${schedule.sid}">${schedule.title}</a>
									</td>
									<td class="text-center">${schedule.sdate}</td>
									<td class="text-center">${schedule.subdate}</td>
									<td class="text-center">${schedule.ecount}人</td>
									<td class="text-center">
									<button id="lsBut-${schedule.sid}" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-cloud-upload"></span>&nbsp;处理申请</button>
									</td>
